...
def setup_venv(build_top="../../../../build"):
    ...
    with lcd(venv_dir):
        with prefix("source %s/bin/activate" % (venv_dir)):
            ...
            local(
                "pip install --upgrade "
                "../../api-server/dist/tungsten-config-api-0.1dev.tar.gz")
            local(
                "pip install --upgrade "
...
def run_tests(build_top="../../../../build"):
    ...
    with lcd(venv_dir):
        with prefix("source bin/activate"):
            local(
                "cp ../../../../../src/cfgm/schema-transformer/"
                "test/test_service.py "
                "lib/python%s/site-packages/schema_transformer/" %
                (pyver))
            local("which python")
            local(
                "python lib/python%s/site-packages/schema_transformer/"
                "test_service.py" %
                (pyver))
# end run_tests
