...
public:
    ...
    bool BulkDataSync() {
        ...
        for (tungsten_rapidjson::Value::ConstMemberIterator k =
             (*events())[tungsten_rapidjson::SizeType(cevent_-1)]
                ["OBJ_FQ_NAME_TABLE"].MemberBegin();
             k != (*events())[tungsten_rapidjson::SizeType(cevent_-1)]
                ["OBJ_FQ_NAME_TABLE"].  MemberEnd(); ++k) {
            std::string obj_type = k->name.GetString();
            for (tungsten_rapidjson::Value::ConstMemberIterator l =
                    (*events())[tungsten_rapidjson::SizeType(cevent_-1)]
                        ["OBJ_FQ_NAME_TABLE"][obj_type.c_str()].MemberBegin();
                 l != (*events())[tungsten_rapidjson::SizeType(cevent_-1)]
    ...
    tungsten_rapidjson::Document *events() {
        if (db_load_.IsArray())
            return &db_load_;
        return &events_;
    }
    tungsten_rapidjson::Document *db_load() { return &db_load_; }
    ...
    static void ParseEventsJson(
        ...
        tungsten_rapidjson::Document *doc = config_cassandra_client->events();
        ...
        if (doc->IsObject() && doc->HasMember("cassandra") &&
            ...
            tungsten_rapidjson::Document *db_load =
                config_cassandra_client->db_load();
            tungsten_rapidjson::Document::AllocatorType &a =
    ...
    static void FeedEventsJson(ConfigClientManager *config_client_manager) {
        ...
        while ((*config_cassandra_client->cevent())++ < events->Size()) {
            ...
            if ((*events)[tungsten_rapidjson::SizeType(cevent)]["operation"]
            ...
            config_client_manager->config_amqp_client()->ProcessMessage(
                (*events)[tungsten_rapidjson::SizeType(cevent)]["message"]
                    .GetString());
        }
...
private:
    ...
    int curr_db_idx_;
    tungsten_rapidjson::Document events_;
    tungsten_rapidjson::Document db_load_;
    size_t cevent_;
};
